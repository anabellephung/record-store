    ...
    <form action="" @submit.prevent="addRecord">
      ...
      <div class="mb-6">
        ...
        <input
          ...
          autofocus 
          autocomplete="off"
          placeholder="Enter year of record"
      ...
      <div class="mb-6">
        ...
        <div class="relative inline-block w-full text-gray-700">
          <select v-model="newRecord.artist" id="artist" class="select" placeholder="Select an artist">
        ...
        <p class="pt-4">Don't see an artist? <router-link class="text-gray-700 underline" to="/artists">Create one</router-link></p>
      </div>

      <input type="submit" value="Add Record" class="font-sans font-bold px-4 rounded cursor-pointer no-underline bg-green-700 hover:bg-green-800 block w-full py-4 text-white items-center justify-center" />
    </form>
      ...
      <li class="py-4" v-for="record in records" :key="record.id" :record="record">
        ...
        <div class="flex items-center justify-between flex-wrap">
          <div class="flex-1 flex justify-between flex-wrap pr-4">
            <p class="font-mono font-semibold flex items-center">
              <svg class="fill-current text-indigo-400 w-6 h-6 mr-2" viewBox="0 0 24 24" width="24" height="24"><title>record vinyl</title><path d="M23.938 10.773a11.915 11.915 0 0 0-2.333-5.944 12.118 12.118 0 0 0-1.12-1.314A11.962 11.962 0 0 0 12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12c0-.414-.021-.823-.062-1.227zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm0-5a1 1 0 1 0 0 2 1 1 0 0 0 0-2z" ></path></svg>
          ...
          <form action="" @submit.prevent="updateRecord(record)">
            <div class="mb-6 p-4 bg-white rounded border border-gray-200 mt-4">
              ...
              <div class="mb-6">
                ...
                <div class="relative inline-block w-full text-gray-700">
                  <select v-model="record.artist" id="artist" class="select" placeholder="Select an artist">
                    <option value="" disabled selected>Select an artist</option>
                    <option :value="artist.id" v-for="artist in artists" :key="artist.id">{{ artist.name }}</option>
                  </select>
                  <div class="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none">

<template>
  <div class="max-w-md m-auto py-10">
    ...
    <form action="" @submit.prevent="addArtist">
      ...
      <input type="submit" value="Add Artist" class="font-sans font-bold px-4 rounded cursor-pointer no-underline bg-green-700 hover:bg-green-800 block w-full py-4 text-white item-center justify-center">
    </form>

    <hr class="border border-gray-200 my-6">

    <ul class="list-reset mt-4">
      <li v-for="artist in artists" :key="artist.id" :artist="artist" class="py-4">
        <div class="flex items-center justify-between flex-wrap">
          <p class="flex-1 font-mono font-semibold flex items-center">
            <svg class="fill-current text-indigo w-6 h-6 mr-2" viewBox="0 0 20 20" width="20" height="20"><title>music artist</title>
              <path d="M15.75 8l-3.74-3.75a3.99 3.99 0 0 1 6.82-3.08A4 4 0 0 1 15.75 8zm-13.9 7.3l9.2-9.19 2.83 2.83-9.2 9.2-2.82-2.84zm-1.4 2.83l2.11-2.12 1.42 1.42-2.12 2.12-1.42-1.42zM10 15l2-2v7h-2v-5z"></path>
            </svg>
            {{artist.name}}
          </p>
          ...
          <button class="bg-transparent text-sm hover:bg-red-500 hover:text-white text-red-500 border border-red-500 no-underline font-bold py-2 px-4 mr-2 rounded" @click.prevent="removeArtist(artist)">Remove</button>
        </div>
        ...
        <div v-if="artist == editedArtist">
          <form action="" @submit.prevent="updateArtist(artist)">
            <div class="mb-6 p-4 bg-white rounded border border-gray-200 mt-4">
              <input type="text" class="input" v-model="artist.name">
